...
import { EDIT_PREVIOUS_DATA_MESSAGE } from "~/constants/travels/report.constant";
...
export const editPreviousDataFlow = addKeyword(EVENTS.ACTION)
    .addAction(async (ctx, { state, gotoFlow }) => start(ctx, gotoFlow, TIMER_END_PROCESS, state, 0))
    .addAnswer(EDIT_PREVIOUS_DATA_MESSAGE,
        {
            capture: true
        },
